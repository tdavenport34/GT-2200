...
t1_handler:
        addi $sp, $sp, -1
        sw   $k0, 0x0($sp)   !move stack pointer up one, store $k0 on it
        ei                   !enable interrupts directly afterwards
               

        addi $sp, $sp, -1
        sw $a0, 0x0($sp)
        addi $sp, $sp, -1
        sw $a1, 0x0($sp)     !store $a0 and $a1 on the stack

                             !state saved, do work
        lea $a0, ticks1      !load the address of ticks1 into $a0
        lw  $a0, 0x0($a0)
        lw  $a1, 0x0($a0)    !load the value from memory, using $a0 as an address, into $a1
        addi $a1, $a1, 1
        sw  $a1, 0x0($a0)    
                             !work done, restore state
        lw $a1, 0x0($sp)
        addi $sp, $sp, 1
        lw $a0, 0x0($sp)
        addi $sp, $sp, 1
...
t2_handler:
        ...
        sw $a1, 0x0($sp)     !store $a0 and $a1 on the stack
                             ...
                             !state saved, do work
        lea $a0, ticks2      !load the address of ticks2 into $a0
        lw  $a0, 0x0($a0)
        lw  $a1, 0x0($a0)    !load the value from memory, using $a0 as an address, into $a1
        addi $a1, $a1, 1
        sw  $a1, 0x0($a0)    
